...
class EditingPipeline(BasePipeline):
    def __call__(
        ...
        # NOTE: prompt_embeds[0] contains the embeddings for the unconditional prompt 
        #######(negative_prompt, or an empty string if not provided).
        ####### prompt_embeds[1] contains the embeddings for the conditional prompt
        ...
        ###### NOTE: 'self.prepare_latents' returns its last argument if it is not None
        ###### We will input x_in with DDIM inversion result 
        ...
        ########## eddit prompt_embeds_edit ##################################
        ########## HINT: add the edit only to the second prompt, idx 0 is the negative (null) prompt
        # FILL 
            ...
            for i, t in enumerate(timesteps):
                ...
                for name, module in self.unet.named_modules():
                    ...
                    if module_name == "CrossAttention" and 'attn2' in name:
                        ...
                        ############ force to match current and reference cross attention map ##
                        curr = # FILL # HINT: reference attention!
                        ref = # FILL #find from d_ref_t2attn  
                        loss += # FILL # use l2 loss and sum all losses over dim 1,2 and then mean over dim 0 
